...
void FullyConnectedLayer::forward(double (*active)(double)){  //计算outputs
...
void FullyConnectedLayer::backward(double (*activegrad)(double)){
	for (int i = 0; i < inputN; i++) {
		double sum = 0;
		for (int j = 0; j < outputN; j++) {
...
void FullyConnectedLayer::update(double alpha){
	for (int i = 0; i < inputN; i++) {
		for (int j = 0; j < outputN; j++) {
